...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        for i, feat in enumerate(self.features):
            distance.append((self.distance_function(point, feat), i, self.labels[i]))
            
        distance = sorted(distance, key=lambda x: (x[0], x[1]))
        
        top_k = [x[2] for x in distance[:self.k]]
        return top_k
    ...
    def predict(self, features):
        ...
        for i, feat in enumerate(features):
            top_k = self.get_k_neighbors(feat)
            c = Counter(top_k)
            predictions.append(sorted(c, reverse=True, key=lambda x: c[x])[0])
